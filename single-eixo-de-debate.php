    ...
    <div class="container">
        ...
        while (have_posts()) : the_post();
            ...
            comment_form(array(
                ...
                <p class="logged-in-as">' . sprintf( __( 'Logged in as <a href="%1$s">%2$s</a>. <a href="%3$s" title="Log out of this account">Log out?</a>' ), get_edit_user_link(), $user_identity, wp_logout_url( apply_filters( 'the_permalink', get_permalink( $post_id ) ) ) ) . '</p>',
                'comment_notes_before' => '<p class="red">A sua participação pode ser através desse campo de comentários para opinar sobre os aspectos do Anteprojeto dentro desse eixo.</p>
                                            <p class="comment-notes"><span id="email-notes">' . __( 'Your email address will not be published.' ) . '</span>'. ( $req ? $required_text : '' ) . '</p>',
